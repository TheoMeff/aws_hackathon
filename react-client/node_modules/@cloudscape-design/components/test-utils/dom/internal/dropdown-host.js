...
class DropdownHostComponentWrapper extends dom_1.ComponentWrapper {
     ...
     * * expandToViewport (boolean) - Use this when the component under test is rendered with an `expandToViewport` flag.
     */
    clickSelectAll(options = { expandToViewport: false }) {
        this.assertOpenDropdown(options);
        const selectAll = this.findDropdown().findSelectAll();
        if (!selectAll) {
            throw new Error('Can\'t select all options, because there is no "select all" option. Make sure that this Multiselect has the `enableSelectAll` property set to true.');
        }
...
__decorate([
    dom_1.usesDom
], DropdownHostComponentWrapper.prototype, "clickSelectAll", null);
