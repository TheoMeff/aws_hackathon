...
try:
    ...
    # Fallback to simpler imports when AWS SDK is not available
    import boto3
    USE_AWS_SDK = False
...
class S2sSessionManager:
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            # Start enhanced transcription with speaker diarization
            self.transcribe_task = asyncio.create_task(self._run_transcribe())
    ...
    async def _run_transcribe(self):
        """Run medical transcription with speaker diarization for clinical conversations."""
        try:
            # Create medical transcriber with speaker diarization
            ...
            logger.info("Starting medical transcription with speaker diarization")

            async for transcript_event in transcriber.run():
                if not self.is_active:
                    break

                # Create conversation line event for the frontend
                conversation_event = self._create_conversation_line_event(transcript_event)

                # Add to output queue for frontend
                await self.output_queue.put(conversation_event)

                logger.debug(f"Medical Transcribe: [{transcript_event['speaker']}] {transcript_event['text']}")

        except Exception as e:
            logger.error(f"Medical transcription error: {e}")
        finally:
            logger.info("Medical transcription stopped")

    def _create_conversation_line_event(self, transcript_data: dict) -> dict:
        """Create a conversation line event from transcript data"""
        return {
            "event": {
                "conversationLine": {
                    "lineId": transcript_data.get("line_id", str(uuid.uuid4())),
